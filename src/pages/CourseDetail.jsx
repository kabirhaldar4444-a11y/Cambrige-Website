...
const getDurationRange = (courseId) => {
  const rangeMap = {
    // Popular programs
    "self-confidencebuildingtraining":          "35 to 50 Days",
    "resiliencecoachtraining":                  "25 to 40 Days",
    "motivationalspeakertraining":              "35 to 50 Days",
    "bodylanguageexperttraining":               "38 to 50 Days",
    "personalbrandingstrategisttraining":       "30 to 45 Days",
    "mindsetmasterytraining":                   "20 to 35 Days",
    "relationshipcoachingtraining":             "18 to 32 Days",
    "decisionmakingmasterytraining":            "28 to 40 Days",
    "adaptiveleadershiptraining":               "25 to 38 Days",
    "theeverydayaitoolkit":                     "15 to 28 Days",
    "confidenceandcharismatraining":            "32 to 48 Days",
    "aiproductivitypowerup":                    "20 to 35 Days",
    // General Professional
    "professionaldevelopmentcertification":     "28 to 42 Days",
    "advancedcareerskillscertification":        "30 to 45 Days",
    "workplaceexcellencecertification":         "22 to 36 Days",
    "professionalgrowthcertification":          "25 to 40 Days",
    "careerdevelopmentcertification":           "32 to 48 Days",
    "workplacereadinesscertification":          "18 to 32 Days",
    // Business & Management
    "certificateinbusinessmanagement":          "35 to 50 Days",
    "certificateinleadershipteammanagement":    "38 to 50 Days",
    "certificateinprojectcoordination":         "28 to 42 Days",
    "certificateinofficeadministration":        "22 to 36 Days",
    "certificateinbusinessoperations":          "30 to 45 Days",
    "certificateinsupervisoryskills":           "20 to 34 Days",
    // Productivity & Office Skills
    "advancedmsexcelcertification":             "18 to 32 Days",
    "businesscommunicationcertification":       "20 to 35 Days",
    "timemanagementproductivitycertification": "15 to 28 Days",
    "professionalemailwritingcertification":    "10 to 20 Days",
    "officeproductivitycertification":          "18 to 30 Days",
    "datahandlingcertification":                "22 to 36 Days",
    // Marketing & Communication
    "digitalmarketingcertification":            "32 to 48 Days",
    "socialmediamanagementcertification":       "22 to 38 Days",
    "contentwritingcertification":              "18 to 32 Days",
    "customerrelationshipmanagementcertification": "28 to 42 Days",
    "brandcommunicationcertification":          "20 to 35 Days",
    "publicrelationscertification":             "25 to 40 Days",
    // Technology & Digital Skills
    "webdevelopmentcertification":              "40 to 50 Days",
    "computerapplicationscertification":        "28 to 42 Days",
    "itskillscertification":                    "30 to 45 Days",
    "digitalskillscertification":               "22 to 36 Days",
    "basicprogrammingcertification":            "32 to 48 Days",
    "websitemanagementcertification":           "22 to 36 Days",
    // Career Development
    "careeradvancementcertification":           "25 to 40 Days",
    "professionalexcellencecertification":      "28 to 42 Days",
    "corporatereadinesscertification":          "20 to 34 Days",
    "industryreadinesscertification":           "22 to 36 Days",
    "employabilityskillscertification":         "18 to 30 Days",
    "workplacesuccesscertification":            "20 to 34 Days",
    // Additional Professional Skills
    "customerservicecertification":             "15 to 28 Days",
    "salesskillscertification":                 "20 to 34 Days",
    "businessetiquettecertification":           "12 to 24 Days",
    "problemsolvingcertification":              "22 to 36 Days",
    "decisionmakingcertification":              "24 to 38 Days",
    "leadershipessentialscertification":        "28 to 42 Days",
  };
  return rangeMap[courseId] || "20 to 35 Days";
};
...
const getDynamicCourseDetails = (matchedCourse, category) => {
  ...
  if (category === "General Professional") {
    paragraphs = [
      ...
      `By choosing this Cambridge Learning Services-recognized certification, you are investing in a future-ready professional framework. This program blends theoretical models with hands-on practice, empowering you to implement the best corporate standards in your daily work operations immediately.`
    ];
    learningAreas = [
    ...
    paragraphs = [
      ...
      `Upon completing this Cambridge Learning Services-aligned program, you emerge as a highly employable, adaptable candidate who is immediately prepared to deliver outstanding corporate outcomes.`
    ];
    learningAreas = [
...
const CourseDetail = ({ courseId: propCourseId }) => {
  ...
  // Course-relevant images keyed by course ID (topic-matched)
  ...
  const getCourseImageUrl = (id) => {
    ...
    // fallback
    return "https://images.unsplash.com/photo-1522202176988-66273c2fd55f?auto=format&fit=crop&w=1920&q=80";
  };

  // Still build ids list for prev/next navigation
  const ids = [];
  popularPrograms.forEach(p => { if (p.id) ids.push(p.id); });
  ...
  let course = courseDetailMap[activeCourseId];

  // If not explicitly mapped in courseDetailMap, search catalog & popularPrograms to auto-generate details
  if (!course && activeCourseId) {
    let matchedCourse = popularPrograms.find(p => p.id === activeCourseId);
    let matchedCategory = '';

    if (!matchedCourse) {
      for (const cat of professionalCatalog) {
        const found = cat.courses.find(c => c.id === activeCourseId);
        if (found) {
          matchedCourse = found;
      ...
      // It's a popular program, let's tag its category as 'Additional Professional Skills'
      matchedCategory = 'Additional Professional Skills';
    }

    if (matchedCourse) {
      const generated = getDynamicCourseDetails(matchedCourse, matchedCategory);
      course = {
        title: matchedCourse.title,
        subtitle: `Advance your leadership, theoretical models, and practical competencies in ${matchedCourse.title}.`,
        price: matchedCourse.price.includes('INR') ? matchedCourse.price.replace('INR', '₹') + '/-' : matchedCourse.price,
        duration: "90 to 120 Days Self-Paced",
        terms: "Study materials shared via email after registration and fee payment.",
        learningAreas: generated.learningAreas,
        paragraphs: generated.paragraphs,
        objectives: generated.objectives,
        keyTopics: generated.keyTopics,
        targetAudience: generated.targetAudience,
        prerequisites: generated.prerequisites,
        careerBenefits: generated.careerBenefits
      };
    }
  }

  // Override duration to show a min-max range if not already defined
  if (course && !course.duration) {
    course.duration = getDurationRange(activeCourseId);
  }
  ...
  return (
    <div className="course-detail-page">
              ...
              <div className="form-group">
                ...
                <input 
                  ...
                  onChange={(e) => {
                    ...
                    if (val.length <= 10) {
                      setPaymentData({...paymentData, phone: val});
                    }
                  }}
                  pattern="[0-9]{10}"
      ...
      <style>{`
        ...
        .ready-banner {
          background: linear-gradient(rgba(9, 36, 27, 0.92), rgba(15, 56, 42, 0.94)), url(${ReadyImage});
        ...
        .payment-modal-overlay {
          ...
          right: 0;
          bottom: 0;
          background-color: rgba(9, 36, 27, 0.88);
          display: flex;
          align-items: center;
          justify-content: center;
          z-index: 9999;
